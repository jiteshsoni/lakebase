...
from src.core.pool import EnhancedProductionConnectionPool
...
class Lakebase1MBenchmark:
    def __init__(self):
        ...
        try:
            ...
            from src.core.secure_credentials import get_secure_credentials, validate_secure_credentials
            
            # Validate credentials are available
            if not validate_secure_credentials():
                raise RuntimeError("Secure credentials not available")
            ...
            self.table_rows = benchmark_config.get('table_rows', 10000000)  # 10 million rows
            ...
            self.table_rows = 10000000  # 10 million rows
    ...
    def run_concurrent_queries(self, thread_count):
        ...
        if pool_key not in self.connection_pools:
            ...
            # Use simple connection approach instead of enhanced pool
            self.connection_pools[pool_key] = None  # We'll use direct connections
        
        connection_pool = self.connection_pools[pool_key]
        
        # No pool initialization needed for direct connections
            ...
            try:
                ...
                while time.time() < end_time:
                    try:
                        ...
                        user_id = local_operations % 10000000
    ...
    def run_full_benchmark(self):
        ...
        print(f"🎯 Starting Lakebase 10M Row High-Concurrency Benchmark")
        print("=" * 70)
    ...
    def generate_summary(self):
        ...
        print("\n" + "=" * 70)
        print(f"🏆 LAKEBASE 10M ROW BENCHMARK SUMMARY")
        print("=" * 70)
        ...
        from src.core.utils import get_results_directory
        results_base = get_results_directory()
        results_dir = f"{results_base}/{timestamp}"
        os.makedirs(results_dir, exist_ok=True)
        
        # Save CSV results
        csv_filename = f"lakebase_10m_benchmark_{timestamp}.csv"
    ...
    def _generate_report(self, report_path, timestamp):
        ...
        with open(report_path, 'w') as f:
            f.write(f"# Lakebase 10M Row Point Lookup Benchmark Report\n")
            f.write(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            
            # Configuration section
            f.write(f"## 🔧 Configuration\n")
            f.write(f"- **Table:** {self.test_table}\n")
            f.write(f"- **Rows:** 10,000,000\n")
            ...
            f.write(f"This tests how well Lakebase handles **concurrent workloads** on a **10M row dataset** with **OAuth rate limiting protection**.\n\n")
            ...
            f.write(f"- **Parameters:** Random user_id values from 1 to 10,000,000\n")
            ...
            f.write(f"\n---\n")
            f.write(f"*Report generated by Lakebase 10M Row Benchmark Tool with OAuth Rate Limiting Protection*\n")
